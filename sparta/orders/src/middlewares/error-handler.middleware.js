...
export default function (err, req, res, next) {
  ...
  if (err.name.includes("ValidationError")) {
    return res.status(400).json({ message: `${err.message} 형식이 올바르지 않습니다` });

  } else if (err.name === "priceValidation"){
    return res.status(400).json({ message: "메뉴 가격은 0보다 작을수 없습니다." });
    ...
    return res.status(401).json({
      message: `${err.message}만 사용할 수 있는 API입니다`
    })
  } else if (err.name ==="needLogin"){
